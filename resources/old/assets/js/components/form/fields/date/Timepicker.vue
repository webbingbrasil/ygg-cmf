        ...
        <div class="dropdown">
            <div class="select-list">
                <ul class="hours" ref="hours">
                    <li :class="{active: value && hour === hr}" @click.stop="select2('hour', hr)"
                        v-for="hr in croppedHours" v-text="hr"></li>
                </ul>
                <ul class="minutes" ref="minutes">
                    <li :class="{active: value && minute === m}" @click.stop="select2('minute', m)"
                        v-for="m in croppedMinutes" v-text="m"></li>
                </ul>
...
<script>
    ...
    export default {
        ...
        watch: {
            minute() {
                this.$nextTick(_ => this.updateScroll('minutes'));
            },
            hour() {
                this.$nextTick(_ => this.updateScroll('hours'));
            },
            active(a) {
                a && ['minutes', 'hours'].forEach(ref => this.updateScroll(ref));
            },
        },
        computed: {
            minMoment() {
