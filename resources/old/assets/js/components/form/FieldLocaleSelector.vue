<template>
    <div class="YggFieldLocaleSelector">
        <template v-for="locale in locales">
            <button
                :class="{
                    'YggFieldLocaleSelector__btn--active': isActive(locale),
                    'YggFieldLocaleSelector__btn--empty': isEmpty(locale),
                }"
                @click="handleButtonClicked(locale)"
                class="YggFieldLocaleSelector__btn ml-2"
            >
                {{ locale }}
            </button>
        </template>
