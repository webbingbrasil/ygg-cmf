...
class Updates
{
     ...
     * @var string
     */
    public $apiURL = 'https://packagist.org/p/webbingbrasil/ygg-cmf.json';
    ...
    public function updateInstall()
    {
        try {
            ...
            for ($i = 0, $max = random_int(10, 20); $i < $max; $i++) {
                $packages[] = ['name' => 'webbingbrasil/ygg-cmf', 'version' => $this->currentVersion.'.0'];
            }
    ...
    {
        try {
            $versions = json_decode(file_get_contents($this->apiURL), true)['packages']['webbingbrasil/ygg-cmf'];
